...
# make the function query1() return the number of votes for the link whose ID is
# 15
...
def query2():
    user_link = [l for l in links if l.submitter_id == 62443]
    return (sorted(user_link, key=attrgetter("submitted_time")))
...
def link_by_id1(link_id):
    for l in links:
        if l.id == link_id:
            return l
print "link_by_id1 --> " + str(link_by_id1(24))
...
def link_by_id(link_id):
    ...
    # IN method (longer and slower):
    # if link_id in link_index:
    #     return link_index[link_id]

print "link_by_id --> " + str(link_by_id(24))
...
add_new_link(Link(50,1,1,1,"title","url"))
